...
const CameraView = forwardRef(
  (
    ...
    const [hasMultipleCameras, setHasMultipleCameras] = useState(false); // Track if device has multiple cameras
    const [isSwitchingCamera, setIsSwitchingCamera] = useState(false); // Track camera switching state
    ...
    useImperativeHandle(ref, () => ({
      ...
      getHasMultipleCameras: () => hasMultipleCameras,
    }));
    ...
    const switchCamera = async () => {
      ...
      const wasStreaming = isStreaming; // Capture streaming state before stopping
      ...
      if (wasStreaming) {
        ...
        // Wait for cleanup and state update
        await new Promise((resolve) => setTimeout(resolve, 500));

        // Start with new camera - the state should be updated by now
        console.log("🎥 Starting camera with new facing mode:", newFacing);
        await startCamera();
      }
    ...
    useEffect(() => {
      if (isActive) {
        ...
        detectCameras(); // Detect available cameras
        startCamera();
      }
    }, []); // Run once on mount

    // Detect cameras when streaming starts
    useEffect(() => {
      if (isStreaming) {
        detectCameras();
      }
        ...
        <Card className="bg-black/50 border-purple-500/30 backdrop-blur-sm overflow-hidden h-full">
          <CardContent className="p-0 h-full">
            <div className="relative w-full h-full bg-slate-900 overflow-hidden">
              ...
              {/* Camera Switching Status */}
              {isSwitchingCamera && (
                <div className="absolute top-6 sm:top-4 left-4 right-4 z-[60]">
                  <div className="bg-blue-600/90 backdrop-blur-sm rounded-lg p-3 flex items-center space-x-3 shadow-xl">
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    <div>
                      <p className="text-white font-medium text-sm">
                        Switching Camera
              ...
              {/* Switch Camera Button - Top Right (when streaming) */}
              {showControls && isStreaming && (
                <div className="absolute top-24 right-4 z-[60]">
                  <Button
                    onClick={switchCamera}
                    size="lg"
                    className="bg-blue-600 hover:bg-blue-700 text-white shadow-2xl border-2 border-white/50 backdrop-blur-md"
                  >
